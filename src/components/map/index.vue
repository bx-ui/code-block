<template>
  <div id="container">
    ...
    <div class="posi-value">
      <el-select v-model="keyword" placeholder="请选择" size="small" @change="changeSelect">
        <el-option v-for="item in mockList" :key="item.id" :label="item.name" :value="item.id" />
      </el-select>
    </div>
    <func-list class="pop-list" />
  </div>
</template>
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      const createMap = (list: Enterprise[]) => {
        ...
        map.centerAndZoom(new BMapGL.Point(113.463981, 30.345058), 11) // 仙桃
