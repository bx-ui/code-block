...
<script lang="ts">
  ...
  import mapOptions from './xiantao_map_coor_option.js'
      ...
      const initChart = () => {
        ...
        const option = {
          ...
          tooltip: {
            ...
            formatter: params => {
              return appleDom + params.name + '<br/>'
            }
          },
          geo: {
            map: 'liangxi',
            ...
            // center: [115.97, 29.71],
            // regions: regionsData, // 在地图中对特定的区域配置样式
          },
          series: [
            {
              // 图表的类型为散点图（地图上的黄色旗子）
        ...
        chartIns.value.on('click', function (params) {
          if (params.componentSubType === 'effectScatter') {
            console.log(params)
            router.push({ name: 'projectDetail', params: { id: params.data } })
          }
        })
      }
      const formatRegionsData = () => {
